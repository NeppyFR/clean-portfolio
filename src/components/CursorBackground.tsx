 ...
 * the existing site.
 *
 * Flag ("shader"): a WebGL fbm-noise plane lit by the cursor. Same palette,
 * different texture. Opt in with <CursorBackground variant="shader" />.
 *
...
const GLOW_RGB: [number, number, number] = [124, 58, 237]; // --color-accent-deep
const DOT_RGB: [number, number, number] = [163, 113, 247]; // --color-accent
...
export function CursorBackground({
  ...
  useEffect(() => {
    ...
    const frame = () => {
      ...
      if (cursor.tx > OFF + 1000) {
        ...
        g.addColorStop(0, rgba(GLOW_RGB, 0.2));
        g.addColorStop(0.5, rgba(GLOW_RGB, 0.07));
      ...
      for (const d of dots) {
        ...
        const driftX = Math.sin(t * 0.6 + d.phase) * 1.4;
        const driftY = Math.cos(t * 0.5 + d.phase * 1.3) * 1.4;
  ...
  return (
    ...
    >
      {/* Base wash — always present, keeps the hero from being flat black
          and guarantees a sane look before/without the interactive layer. */}
      <div
        className={
          reduced
      ...
      {!reduced && variant === "shader" && <ShaderBackground />}
